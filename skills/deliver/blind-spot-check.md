...
Read the sampling rate from `evaluators.spot_check.rate` in `orchestrate.json` (integer N — review every Nth converged bean). If the key is absent, default to `5`. If the value is `0` or less, the spot-check is disabled; return immediately.
...
<HARD-GATE>
For each sampled bean, you MUST present the raw diff and collect the human's scores BEFORE revealing the evaluator scorecard.

  1. Show the human the raw diff for the bean only:
       git diff {BASE_SHA}...HEAD   (BASE_SHA from the bean's Evaluation Log)
     Do NOT show, summarize, paraphrase, or hint at the evaluator's scores, evidence, guidance, or verdict.
  ...
  4. ONLY AFTER the human has committed their scores, reveal the evaluator scorecard for the same bean.

Do NOT reveal the evaluator scorecard, or any part of it, before the human's scores are recorded.
Do NOT reorder these steps. Blind ordering is the entire point — an anchored human measures nothing.
</HARD-GATE>
...
- **Never** show any part of the evaluator scorecard, evidence, or verdict before the human commits their blind scores
- **Never** score the dimensions yourself — the human must score the diff cold
- **Never** invent a new divergence format — reuse the attended-gate correction shape and anchor format verbatim
- **Never** skip logging divergences — they must land in the eval log and the calibration file, not just the summary
