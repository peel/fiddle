...
An entry is a **path** if it contains `/` and ends in `.rs`, `.sh`, `.toml` or `.md`, and it must name a file that exists — `workspace/command.rs` resolves as a suffix, so a partial path is enough and `target/` cannot satisfy one. Anything else is a **symbol**, and it must appear in the text of some file **anywhere in the repository** — the check greps from the root, excluding `.git`, `target`, `.beans`, `node_modules` and `decisions/`, so an entry in `docs/` or `scripts/` resolves. A `.patch` or `.yml` entry is **not** a path: it falls through to the symbol branch, where the literal filename is grepped and fails. Cite a symbol inside such a file instead. `cve/project.rs::names_a_fix` is a symbol: the check reads what follows the last `::`. Getting that distinction wrong is what shipped a red gate — a real file that no file happens to mention failed a content grep.
