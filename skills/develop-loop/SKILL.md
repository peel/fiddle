...
| "Scorecard looks good, skip merge scripts" | You cannot eyeball normalization and cross-domain merge. Run merge-scorecards.sh. |
| "Evidence pack is overhead, evaluator can look around" | The evaluator interprets evidence; it does not gather it. Capture the pack first. |
...
Store the resolved domains list for use in steps 1e-2 (evidence pack) and 1f (per-domain evaluator dispatch).
...
<GATE>Proceed to evidence gathering (1e-2), then evaluator dispatch (1f). Implementer DONE is not evaluation.</GATE>

## 1e-2. Gather Evidence Pack (per domain)

For each resolved domain, before any evaluator dispatch:
1. Start runtimes if configured (Runtime Start gate below).
...
4. Probe the running app for runtime-configured domains (existing runtime
   evidence protocol: `skills/runtime-evidence/SKILL.md`) into
   `evidence-{domain}-runtime.txt`.
...
If a domain has runtime configured (i.e., the resolved domain entry has a "runtime" array):
  ...
  2. If exit 0: runtime is ready. Proceed to evidence capture and evaluator dispatch.
  3. If exit 3 (harness failure): retry once. If retry fails, escalate to human without counting against dispatch budget.
  4. If exit 1 or 2: this is an app/config issue. Include the error in the evidence pack and evaluator context.
Do NOT skip runtime start. Do NOT capture runtime evidence or dispatch an evaluator if runtime is required but not started.
Runtimes stay RUNNING through evaluator dispatch (step 1f) — the evaluator may interact with the running app. They stop at the Runtime Stop gate at the end of 1f.
</HARD-GATE>

## 1f. Dispatch Per-Domain Evaluator

For EACH resolved domain (from `resolved-domains.json` produced in step 1c), dispatch ONE evaluator. Process domains in `runtime_order` if specified, otherwise in the order listed in the `domains` array.
...
**If the selected provider is `claude`:** Dispatch an evaluator subagent using `skills/evaluate/SKILL.md` protocol with that domain's template (e.g., `skills/evaluate/evaluator-general.md`, `evaluator-frontend.md`, or `evaluator-backend.md` — as specified in the resolved domain's `template` field), passing the evidence pack file `evidence-{domain}.txt` in context.

**If the selected provider is external (not `claude`):** Dispatch via the provider hook:
...
The external provider receives the same context as the claude evaluator: evaluation protocol, domain template, calibration data, diff, criteria, and evidence pack. The external provider MUST return a valid JSON scorecard as its last content block (see `skills/develop/provider-context.md` for schema requirements).
...
The evaluator (regardless of provider) returns a single scorecard JSON containing both per-dimension scores (under `.domains`) and pass/fail criteria (under `.criteria`). The scorecard MUST include a `"provider"` field identifying which provider produced it. Save the scorecard per domain:

```bash
# Save the domain's evaluator scorecard (provider from selected-provider.json)
cat > scorecard-{domain}-{provider}.json   # ← evaluator output for this domain

# Increment dispatch_count for the evaluator dispatch
...
Normalize each domain's evaluator scorecard (Per-Domain Normalization) and merge across domains following: `skills/develop-loop/scorecard-merge.md`
...
The --dispatches {count} MUST reflect actual dispatches (each implementer or evaluator dispatch = 1), not just iterations.
For example, one implementer + one evaluator per domain over 2 domains = 3 dispatches for the iteration.
Do NOT pass --disagreements on the per-task path — a single evaluator per domain produces no disagreements file (disagreement tracking applies only to holistic review).
Record the selected provider and reason from selected-provider.json in the log entry — this captures fallback substitutions.
