...
//! `decision` holds the identity of the question put to that person and the
//! marker by which a later process finds it again, `outcome` holds how
...
pub use decision::{
    decision_request_id, parse_marker, render_marker, DecisionBinding, DecisionRequestId,
    MarkerError, MARKER_VERSION,
};
