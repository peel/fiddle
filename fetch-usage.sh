#!/bin/sh
# Fetches Claude API usage stats and caches them.
# Line 1: five_hour.utilization (integer %)
# Line 2: seven_day.utilization (integer %)
# Line 3: five_hour.resets_at (ISO timestamp)
# Line 4: seven_day.resets_at (ISO timestamp)

CACHE_FILE="/tmp/.claude_usage_cache"
TOKEN_CACHE="/tmp/.claude_token_cache"
TOKEN_TTL=900

# Use /bin/date and /usr/bin/stat explicitly (macOS BSD) to avoid nix/GNU shadows
DATE=/bin/date
STAT=/usr/bin/stat

# --- get token (15-min cache) ---
token=""
