...
- **Acceptable (4–7).** Trusted values reach tools only through Rig's host-only `ToolContext` via
  ...
  `read_file`, `edit_file`, `write_file`, `list_files`, `run_check`, and `run_command` where and only
  where the
...
## correctness — Initial Anchor (2026-08-19, M4c: CVE mitigation without an ecosystem)
**Poor (3-4):** Go knowledge is moved rather than deleted — a `cve/ecosystem.rs` with a `Go` variant, or a trait whose only implementor knows `go.mod`. The agent is still handed a target or a version and asked to execute it. The declared-files rule exists but the diff is never actually compared against the claim, so an undeclared edit lands. A Python fixture exists and passes only because a production branch tests for it.
**Acceptable (6-7):** The five Go modules are deleted, the agent decides target and version, and the declared-files rule rejects an undeclared edit. The Python and Go lanes both pass the same code path. Some ecosystem shape survives in a comment or a variable name, and the deletion is proved by a grep for `go.mod` rather than by the Python lane.
**Excellent (9-10):** No production file can say what ecosystem it is operating in, and the Python lane would red if one could. The declared-files rule is inverted in place — an undeclared edit and a declared-but-untouched file each red by name, and neither inversion fails to compile. A declined finding publishes evidence and does not fail the run. What was lost is written where a reader meets it, not only in the spec: the manual says a deployment's check list is now what stops a silenced test.

## domain_spec_fidelity — Initial Anchor (2026-08-19, M4c: CVE mitigation without an ecosystem)
**Poor (3-4):** Rust still refuses something about versions — a major-bump guard, a same-minor preference, a semver parse used to reject. Grouping survives under another name. The rescan is no longer the verdict because a new pre-check filters findings first. M0–M3 lanes were modified to accommodate the refactor.
**Acceptable (6-7):** Rust refuses nothing about versions, there is no grouping, and the rescan decides. The all-or-nothing verdict is implemented as specified. The three owed ADRs exist but one of them restates the spec rather than recording what a reader who knew the old rule now needs.
**Excellent (9-10):** Every guarantee the spec keeps is kept and every one it drops is dropped, with the drops **named where they were previously promised** — §2.5's forbidden-edit paragraph and §2.4's version rules are corrected in the design document rather than left describing deleted code. The ADRs record why the M4a line was defensible and why it moved, so the next capability does not re-derive it wrongly. The unowned registry gap is closed by deletion and the record says so.

## correctness — Initial Anchor (2026-08-26, M5a: Jira observation)
**Poor (3-4):** The port answers `Available` when the site refused the credential, so a 401 reads as an issue with no status. A pre-fetched answer is stored and the second observation returns it, so re-derivation reads a cache rather than Jira. The status is normalized, title-cased or trimmed before it is reported, so the port no longer reports the source's own status verbatim. `revision` stays `None` and the issue `version` is dropped, so a later target identity can name an issue and not a state of it. The stub is a hand-written fake of `JiraHttp` rather than an HTTP server, so nothing parses a real response.
**Acceptable (6-7):** `work_item_port_contract` passes over a loopback HTTP server for all three worlds. A 401, a 403 and a 404 each answer `Unavailable` with a distinct reason. The status is verbatim and `revision` carries the version. The async change compiles and every call site awaits. The connection-refused case is covered by an assertion that would also pass if the reason were empty.
**Excellent (9-10):** Every `Unavailable` reason is asserted by content, not by variant, so a reason that stopped naming the site reds. The re-derivation property is proved by a test in which the server changes its answer between the two observations and the run sees the second answer. The M0 serialization pin still passes and a test says why: the new field is absent for a stub observation, and flipping `skip_serializing_if` reds it. A sentinel `JIRA_API_TOKEN` is searched for in stdout, diagnostics and the bundle and found in none.

## domain_spec_fidelity — Initial Anchor (2026-08-26, M5a: Jira observation)
**Poor (3-4):** M5a writes to Jira — a transition, a comment, a label, or a ticket. `impl AdapterError for JiraError` is added with no effect that uses it, or `JiraHttp` joins `EffectContext` unused. `acli` is spawned anywhere. A Jira crate is added as a dependency. The typed projection exists and no reader prints it, so it is an unused abstraction the milestone was told not to build.
**Acceptable (6-7):** M5a performs reads only, the adapter is `reqwest` against REST v3, and the projection reaches `fiddle inspect`. The ADR exists and records that `acli` was rejected. The out-of-scope list is honoured. The ADR restates the decision without recording that ADR 015's containment property is lost.
**Excellent (9-10):** The ADR names what is given up, not only what is chosen: ADR 015's one-screen property does not survive a library, and the ADR says which discipline replaces it and that a discipline is not a boundary. The status projection's `Unknown` arm is proved to be reached rather than assumed, and the duplicate-name pitfall has a test that reds if the lookup keys on name alone. The live read lane prints the real issue JSON, so the next milestone designs its write against a measurement rather than the documentation.

## code_quality — Initial Anchor (2026-08-26, M5a: Jira observation)
**Poor (3-4):** `JiraHttp` derives `Debug`, so the credential can reach a log through a format string. The token is held as a `String` field beside other configuration. Error text reaches a diagnostic without redaction or a length bound. The async conversion is done by adding `block_on` at one call site rather than by making the trait async.
**Acceptable (6-7):** One construction holds the credential, the type derives neither `Debug` nor `Serialize`, and error text passes redaction. The trait is async and no `block_on` appears. The `jira` module splits http, work_item and status into separate files.
**Excellent (9-10):** The credential cannot be printed by construction rather than by convention, and a compile-fail test proves it — the `AuthorizedEffect` doctest is the pattern. The redaction is asserted against a response body that contains the token, not against a synthetic string. `ports()` selecting on scheme is exhaustive with no wildcard arm, matching ADR 022's rule for capability resolution.
