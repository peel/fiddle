...
**A translating gateway makes a real-model lane load-bearing.** Tool calls go out in OpenAI function-calling shape and are translated to the upstream provider. The deterministic suite structurally cannot see a translation defect, because `MockCompletionModel` replaces the provider entirely and never serialises a request to anyone. Nor can `crates/fiddle-acceptance/tests/binary_repair.rs`, which does serialise real chat-completions requests but answers them from its own loopback socket: it proves our client speaks the wire format, never that the gateway's translation of it is faithful. Only Tier 1 and Tier 2 reach the gateway. This is not a theoretical concern: it is exactly how the structured-output/tool-use defect below was found, and exactly how it would otherwise have shipped green.

**Structured output and tool use interact badly on a compatible endpoint.** Measured directly at the gateway: `tools` alone comes back `finish_reason: tool_calls` with a call; `tools` plus `response_format: {type: json_schema}` comes back `finish_reason: stop`, `tool_calls: null`, with the report filled in from nothing. That measurement stands.

The mechanism this ADR originally gave for the fix does not. It said `OutputMode::Auto` resolves per provider via `composes_native_output_with_tools()`, that `openai::completion::CompletionModel` reporting `true` is false of a compatible endpoint fronting Anthropic, and that `OutputMode::Tool` is "therefore stated explicitly in `agent::attempt`" at the cost of being best-effort where Native was guaranteed.

**`OutputMode::Tool` is inert on this path, and the cost is not being paid.** `rig_agent`'s `TypedPromptRequest::from_agent` overwrites the agent's `output_mode` with `Native` unconditionally, so the builder line changes nothing. `binary_repair::the_serialized_request_offers_four_tools_and_carries_no_host_fact` reads the bytes the compiled binary puts on its socket and pins what actually goes out: **four tools on every turn, no synthetic output tool on any turn, and the native `response_format` constraint on the finalising turn only.** Deleting the line produces byte-identical traffic. The report is still validated afterwards and a malformed one is still `AgentError::Protocol` — but by Native's own path, not by a Tool-mode fallback.

**What made the tool loop start working is not established.** Commit `e993f4a` added the inert line *and* changed `DEFAULT_MODEL` from `claude-haiku-4-5` to `bedrock/moonshotai.kimi-k2.5` in one commit. The line is now known inert, which leaves the model change as the only remaining candidate — and the table below is consistent with it, since both Claude-family models finalise after a single tool call while the other three drive the loop. That is inference from a table measured *after* the change, not an isolated experiment. The honest statement is that the loop works, the wire shape is pinned by a committed test, and the cause was never isolated.

The line is left standing rather than deleted: removing it is a behaviour change that only a real-model measurement per mode across the table below could justify, and no such measurement exists. `docs/BACKLOG.md` (2026-08-09) records that cost.
...
**It is not distinguishable today.** Recorded here as open rather than asserted as satisfied policy, because an accepted ADR claiming a requirement it does not meet is worse than one that admits the gap. `AgentError` has four variants — `Bounded`, `Cancelled`, `Protocol`, `Provider` — and `agent::classify` matches Rig's *typed* variants rather than its message text, deliberately. A spend-cap refusal from the gateway is an HTTP error with no typed variant of its own, so it falls to the wildcard arm as `Provider { reason }`.

**And that `reason` does not carry the response body.** Commit `4b2333b` reduced `agent::provider_fault` to `the gateway answered <status>` for any error that has a status at all, because a gateway error body is precisely where a credential echo surfaces — the leak this milestone closed at three boundaries. `scripts/tier2.sh` still records the outcome kind and the reason — in full in the JSON record, truncated to 300 characters only when printing — but the reason it now records is that one sentence. So a human reading a Tier 2 artifact can separate an exhausted budget from a broken capability only if the gateway answers spend-cap refusals with a status code it uses for nothing else, which this project has never observed. Read the earlier form of this paragraph, which claimed a human could tell them apart from the reason text, as withdrawn: nothing machine-readable can, no human reliably can, and Tier 1 does not classify at all.

The two gaps are not in tension and have one fix. Suppressing the body was right and stays; what is missing is a *typed* signal beside it, which is the opposite of parsing prose.
