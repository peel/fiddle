...
A deployment naming `required_checks = ["build"]` requires nothing of CI: the run reports the name and never requires it. That is the shape `agent.max_capability_attempts` shipped, and ADR 013 had to price it after the fact. One difference mattered for how this key is disclosed. Nothing read `max_capability_attempts` when this was written; ADR 037 later made it fire. This key is read, acted on and published; what it does not do is decide.
