...
- A mitigate run files a ticket only when `[jira.filing]` names a project, an issue type and a ledger issue. A deployment that names no such table sends no request and writes `{"filing": "not_configured"}`, which is a different document from a run that had nothing to file (ADR 080).
- A tracker that refuses a ticket does not fail the run. The refusal is written beside the repair as `TicketFiled::Refused` and is never a `CapabilityError::Write` (ADR 080).
...
- **The Jira indexing lag is bracketed by one run and is characterised by nothing.** `scripts/live-jira-write.sh` ran twice on 2026-08-28 and filed two tickets, `ISP-272` and `ISP-273`, because run two searched for the marker run one had written and matched nothing. The lane reported `0 seconds` for the lag and, in the same run, reported one issue where two existed. The two numbers cannot both be true, so the lane's own number is unsound and must not be quoted. `the_search_then_create_protocol_files_a_second_issue_where_the_claim_ledger_files_one` drives both protocols against one stub inside that window and counts two creates against one. The rewritten lane refuses to print a lag unless the search it read agrees with the number of creates it made, and on 2026-08-29 it bracketed one run's lag at more than 634 ms and at most 1940 ms. That is one observation of one project on one day. No number bounds the lag across runs. The lag no longer bounds the exactly-once claim across an interruption: a run arriving inside the window reads the claim on the ledger issue and sends no second create, and a claim naming no issue is refused rather than created a second time (ADR 079). What the lag still bounds is how long a claim naming no issue stays unresolved (`fiddle-jh1z`).
...
- **An exhaustiveness test is now held to the size of its enum, and still not to its cases.** Such a test claims that every variant is pinned. Against a second hand-written list it proves only that two lists agree. `fiddle-0lcc` swept that shape at `d04d7bd`: where an enum carries an `ALL` constant the test asserts the difference against `ALL` empty, and where it carries none the list length is held to the in-workspace `VariantCount` derive, at compile time wherever the list is an array. `ALL` itself is held to `VARIANT_COUNT`, so the hole does not move into it. MEASURED by counting the tree on 2026-08-29: 22 enums under `crates/*/src` derive `VariantCount`, in 14 files, and two more derive it in the test support. The two lists that were false while green are now true: `crates/fiddle-runtime/tests/workflow_capability.rs:612` compares `failures().len()` with `EffectError::VARIANT_COUNT`, and `parses_every_known_scheme` in `crates/fiddle-core/src/identity.rs:210` lists `("cve", InvocationScheme::Cve)` and filters its examples against `InvocationScheme::ALL`. What the derive closes is the gap between a list and its enum. It does not check that a case is the right case, and a list that subtracts from the count is trusted for the subtraction. MEASURED by sweeping the tree on 2026-08-29: two of the 29 `VARIANT_COUNT` references size a list by subtracting, both state in their own assertion message what they subtract and why, and both subtracted cases are covered by a sibling test, so the sweep found no untrusted subtraction. `every_unsuccessful_arm_is_its_own_error` in `crates/fiddle-runtime/tests/scanner.rs:65` is sized `ScanError::VARIANT_COUNT - 1` and drives the five arms a present scanner can take; its assertion message calls `ScanError::Missing` "the one variant no stub can reach, so it is tested where the scanner is absent", and `a_scanner_that_is_not_installed_is_its_own_classification` at `crates/fiddle-runtime/tests/scanner.rs:32` covers it. `every_non_allow_decision_explains_itself` at `crates/fiddle-core/src/policy.rs:84` subtracts the one `Allow` pair of a cross product, and `crates/fiddle-core/src/policy.rs:50` covers that pair.
- **A `fiddle` binary files a verdict, the whole route from `fiddle.toml` is measured through the binary, and no run of the binary has reached a real site.** `CveMitigate` calls `ticket_proposals` and executes each `FileVerdict` through the executor, and it does so only when `[jira.filing]` is configured; a deployment that names no such table writes `{"filing": "not_configured"}` to `filings.json` and sends no request (ADR 080). A refusal from the site is recorded as `TicketFiled::Refused` beside the repair and does not fail the run, so a misconfigured project files nothing until somebody reads that file. `human::publish`, the other route a Jira write could take, still has no caller outside tests. Every claim in this paragraph so far is measured against the loopback stub in `crates/fiddle-runtime/tests/cve_filing.rs`. The live proof of 2026-08-29 does not cover this bullet: `crates/fiddle-runtime/tests/live_jira_filing.rs` builds a `TicketFiling` itself and calls `ticket_proposals` and the executor as `CveMitigate::file` does, so it measures `FileVerdict` against Atlassian and says nothing about the mapping from `fiddle.toml` to `TicketFiling` in `crates/fiddle-cli/src/main.rs`. MEASURED on 2026-08-29 through the binary and against a loopback stub: `the_filing_table_in_the_document_reaches_the_ticket_the_binary_files` in `crates/fiddle-acceptance/tests/cve_mitigation.rs` runs `fiddle run cve --capability cve_mitigate` over a document carrying `[jira.filing]`, and the project, the issue type and the ledger issue that table names arrive at the site in the create body and the ledger claim path, and derive the marker `filings.json` records. Each of the three was proved falsifiable by changing that one field in the document. `crates/fiddle-cli/src/config.rs` now holds the one mapping both surfaces use, so the table `fiddle config check` echoes is the `TicketFiling` the sweep receives rather than a second reading of the same document. STILL NOT REACHED: no `fiddle` binary has written to a real Jira site, so the agreement between that stub's routes and Atlassian's is argued from M5b's four live lanes and is not measured through the binary; `docs/BACKLOG.md` records why the live run was deferred and what it would cost. The only run path to `FileVerdict` through the binary is a full CVE sweep needing a scanner, an agent and a GitHub repository, all three of which the acceptance harness stubs and a live run would not. `fiddle-2690` weighed that evidence and retired the host step on it: `docs/technical/host-workflow-m4b.patch` now records the verdict mapping step as removed, and the Jira step as keeping only the pull request linking, which fiddle does not do. The patch names what a host must add before it removes them — a `[jira.filing]` table, a `[jira]` table, the two credential variables that table names, and a ledger issue the site already holds — because a host that removes the steps and forgets any of those stops filing tickets and reports it only in `filings.json`.
- **Three of the ten registered effects have no capability that constructs them, and one missing hop explains all three.** `jira.comment_added`, `jira.issue_transitioned` and `jira.pull_request_linked` are 3 of the 10 names in `registry::BUILT_IN`. READ OFF THE BUILD at `25e6239`: `AddComment`, `TransitionIssue` and `LinkPullRequest` are constructed under `crates/fiddle-runtime/src/` only inside `#[cfg(test)]` modules, and each of the other seven names is reached from a capability in `crates/fiddle-runtime/src/capability/`, directly or through one helper. Each of the three acts on an existing issue, so its identity is built from the issue key and the revision the issue was read at (ADR 078), and `FromStepParams` refuses on exactly that ground. A `Step::Effect` in a workflow document therefore constructs none of them: `StepParams` names no issue key and no `fields.updated`, and `neither_effect_is_buildable_from_a_step_alone` holds two of the three refusals. The revision is not missing from the run. `orchestration::run` observes the work item before the capability executes, and `JiraWorkItemPort::observe` answers a `revision`. MEASURED on 2026-08-29 against a loopback stub: `the_revision_a_run_observes_is_the_revision_the_link_builds_its_identity_from` in `crates/fiddle-runtime/tests/jira_comment_effects.rs` gives `LinkPullRequest::new` the observed revision and the raw `fields.updated` in turn and counts one comment request between them, and counts two when a revision the run did not observe is given instead. What is missing is the hop: `Capability::execute` receives the change-set id and the invocation reference and not the observed work item, so no capability holds a revision. `human::publish` is blocked by the same hop on its Jira arm, and its GitHub arm is duplicated by `ProposeChange::ask`, which does not call it because `publish` widens the error with two arms one GitHub channel cannot reach and hides the effect name the receipt evidence line spells. `fiddle-jgnc` decided that the caller for `jira.pull_request_linked` is not built in M5b-wire and wrote the reasoning into the M5c epic body, which also records that the host workflow's remaining link step wants a second caller in `CveMitigate` and not this one.
