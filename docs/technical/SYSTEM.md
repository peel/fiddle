...
| scanner | `fiddle-runtime/src/scanner` | runs `wizcli`, records version and digest | the digest, never the tag it was handed (ADR 020); the caller logs `wizcli` in, and an absent login refuses the scan (ADR 042) |
