...
**`fiddle.toml`** (TOML, `deny_unknown_fields`) — The binary's deployment document. `[agent]` names `model`, `base_url`, `api_key = { env = "NAME" }` and the inner bounds (`max_turns`, `max_tokens`, `max_changed_files`, `deadline`, `tool_timeout`); `[workspace]` names `root`, `isolation`, `command_timeout`, `cleanup`, the `fixture` under repair and the `check = { program, args }` that decides its outcome. `api_key` deserializes only from `{ env = … }`, so a document holding a literal secret does not load.
...
**M1 commands** — Three tiers, and only the first one gates.

```
# deterministic: free, offline, MockCompletionModel, always runs (part of the gate)
...
- Workspace commands run under `env_clear` with an explicit allowlist: **a locator may be inherited, an authority may not.** `PATH` and `RUSTUP_HOME` say where a toolchain is; `CARGO_HOME` and every credential say what may be done with it, and are not passed.
...
- `git status` runs with `-uall`. Under an operator's `status.showUntrackedFiles=no`, or git's default untracked mode over a wholly-new directory, the changed-file evidence silently empties or under-reports.
...
- No model-visible surface carries the host layout — not a tool's JSON schema, not an error message, and not a tool's **success output**. Host facts reach a tool through `ToolContext`; a schema is a menu, and anything named on it is something the model may fill in.
