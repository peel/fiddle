...
| transcript | `fiddle-runtime/src/agent/transcript.rs` | records what the model was sent and what it returned, off unless `FIDDLE_TRANSCRIPT=1` | every text passes through `Redaction`, and a run that wrote one says so (ADR 052) |
