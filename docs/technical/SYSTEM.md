...
- `jira.issue_filed` deduplicates on a marker label written in the create call. Zero matches create, one match reads, and two or more refuse the write and create nothing (ADR 079).
...
- **`jira.issue_filed` cannot land against a real Jira site.** Three defects are independent of each other. MEASURED on `snplow.atlassian.net` on 2026-08-28: `GET /rest/api/3/search/jql` answers each issue with `id` and no `key` unless the caller asks for `fields=key`, and `createmeta` for `ISP` lists `issuetype` among the required fields of a `Task`. ARGUED from those measurements and from the code: `FileVerdict` asks for no fields and reads `key` on every result, so `inspect` answers `JiraError::Malformed` and the effect can never recognise a ticket it filed; and `FileVerdict::body` sends no `issuetype`, so the create would be refused. No live run has driven `FileVerdict` itself. `crates/fiddle-runtime/tests/support/stub_jira.rs` answers both `id` and `key`, so no hermetic lane reds. Bean `fiddle-jh1z` carries the fix, and ADR 079 carries the record.
- **The Jira indexing lag is unmeasured, and the exactly-once claim is bounded by it.** `scripts/live-jira-write.sh` ran twice on 2026-08-28 and filed two tickets, `ISP-272` and `ISP-273`, because run two searched for the marker run one had written and matched nothing. The lane reported `0 seconds` for the lag and, in the same run, reported one issue where two existed. The two numbers cannot both be true, so the lane's own number is unsound and must not be quoted. `a_fresh_process_inside_the_lag_window_files_a_second_issue_and_the_next_read_refuses_both` states the bound; no number describes it (`fiddle-jh1z`).
...
- **The verdict rows are proposals that nothing files.** `ticket_proposals` in `crates/fiddle-runtime/src/cve/verdict.rs` builds a `TicketProposal` for each verdict row that carries a legacy label, and no run path calls it. `human::publish` has no caller outside tests. So no `fiddle` binary performs a Jira write, and `docs/technical/host-workflow-m4b.patch` keeps its verdict mapping step and its Jira step (`fiddle-zlc4`).
