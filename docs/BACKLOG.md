...
**2026-08-09 — `agent.max_capability_attempts` has no consumer** is closed as a visibility matter. It stays open as a behaviour one. The retry loop is still not built, and ADR 013 still prices it. A document writing `max_capability_attempts = 5` is no longer told it is simply valid. `config check` reports `{"configured": 5, "enforced": 1, "status": "accepted-not-enforced", "decision": "013-one-attempt-bound-not-two"}` in `--json`, and says the same in prose. Every bound that fires stays a plain scalar, so the shape tells the two kinds apart. Design §6.6 promises a deferred key is loud rather than silent under `deny_unknown_fields`. This key escaped that by being known rather than unknown. ADR 013's consequences section is corrected: it asserted the edge was findable in "exactly two places" and "not surfaced at runtime". The `#[allow(dead_code)]` is gone with it.
