...
The `OutputMode` consequence keeps the gateway measurement and retracts the mechanism. `TypedPromptRequest::from_agent` overwrites the mode with `Native` unconditionally, so the builder line is inert. The "Tool mode is best-effort where Native was guaranteed" cost is not paid. `binary_repair::the_serialized_request_offers_five_tools_and_carries_no_host_fact` pins what goes out. The attribution now names both changes that landed in `e993f4a`: the inert line, and `DEFAULT_MODEL` moving from haiku to kimi. It says the cause was never isolated. The budget consequence states what `tier2.sh` records after `4b2333b`: `the gateway answered <status>` and nothing else. Item 8 went with it: the inline comment at the `.output_mode` call now says the line is inert.
...
`[[workspace.commands]]` gives an attempt a fifth tool, `run_command`, wherever a deployment declares a program for it to run. ADR 044 records the decision. Two entries in this file state a count that is now conditional, and this entry names them rather than rewriting them.
